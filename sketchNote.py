...
def process_image(image_path, target_width=160, target_height=160):
    """Resize the input image to 160x320 and return RGB pixel matrix."""
...
def create_musicxml_template(total_measures=400):
    """创建一个MusicXML模板文件，包含400个小节，每行5个小节，每页40行。"""
    ...
    ET.SubElement(page_layout, 'page-height').text = "2000"  # 页面高度
    ET.SubElement(page_layout, 'page-width').text = "1200"   # 页面宽度
    ...
    ET.SubElement(system_layout, 'system-distance').text = "0"  # 行间距完全消除
    ET.SubElement(system_layout, 'staff-distance').text = "0"   # 五线谱距离完全消除

    # 隐藏五线谱，通过颜色为白色和将staff-lines设为0确保不可见
    staff_layout = ET.SubElement(system_layout, 'staff-layout')
    ET.SubElement(staff_layout, 'staff-lines').text = "0"  # 设置五线谱线数为0
    ET.SubElement(staff_layout, 'staff-line-color').text = "#ffffff"  # 设置五线谱颜色为白色

    # 生成400个小节，每行5个小节，每页40行
    for measure_number in range(1, total_measures + 1):
        measure = ET.SubElement(part, 'measure', number=str(measure_number))

        # 在每个小节添加乐谱属性，并固定小节宽度为64个spatium
        attributes = ET.SubElement(measure, 'attributes')
        measure_width = ET.SubElement(attributes, 'measure-width')
        measure_width.text = "64"  # 固定小节宽度为64个spatium

        # 在第一个小节添加乐谱属性
        if measure_number == 1:
            divisions = ET.SubElement(attributes, 'divisions')
            divisions.text = "64"  # 使用64分音符
        ...
        if measure_number % 10 == 0:
            ...
            ET.SubElement(system_layout, 'system-distance').text = "0"  # 行间距为0
...
def add_blank_measures(score, count=9):
    """Adds blank measures at the beginning of the score with white-colored notes."""
...
def map_pixels_to_notes(score, color_image):
    """Maps pixels of a 160x160 image to MusicXML notes across 10x8 measures."""
    part = score.find('part')
    measures = part.findall('measure')
    
    # Ensure correct image and measure setup
    ...
    # Start filling the measures from the 10th one onward
    for measure_col in range(10):        # Across 10 columns of measures
        for measure_row in range(8):     # Across 8 rows of measures
            measure_index = (measure_row * 10 + measure_col) + 9  # Offset by 9 for the blank measures
            ...
            # Map each group of 16x20 pixels to notes
            for chord_index in range(16):
                ...
                for note_offset, (step, octave) in enumerate(note_positions):
                    ...
                    # Add chord tag to additional notes in chord
                    if note_offset > 0:
